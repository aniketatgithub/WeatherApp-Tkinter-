...
# Define JSON schema for current weather data
    ...
    "properties": {
        "data": {
            ...
            "items": {
                ...
                "required": ["city_name", "country_code", "temp", "weather"]
            }
        }
    },
    "required": ["data"]
    ...
    "properties": {
        "data": {
            ...
            "items": {
                ...
                "required": ["datetime", "temp", "rh", "wind_spd", "weather"]
            }
        }
    },
    "required": ["data"]
...
def update_ui_with_error(message):
        city_info.config(text=message, fg='red')
...
def world_weather():
    ...
    root = Tk()
    root.title('World Weather by aniket')
    ...
    def get_current_weather(key, location):
        try:
            ...
            validate(instance=current_data, schema=current_weather_schema)
            return current_data
        except requests.exceptions.JSONDecodeError:
            raise Exception("Failed to decode JSON.")
        except ValidationError as e:
            raise Exception(f"JSON data does not conform to expected schema: {str(e)}")
    ...
    def get_forecast_weather(key, location):
        try:
            ...
            validate(instance=forecast_data, schema=forecast_weather_schema)
    ...
    aniket = Label(text='by aniket ', font=('Noto Sans', 8), bg=l_blue, fg='white')
    aniket.place(x=143, y=300)
